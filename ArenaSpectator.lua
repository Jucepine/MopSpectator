...
local PLAYERPETS = {
    ...
    [49] = "spell_frost_summonwaterelemental_2",
    [50] = "inv_misc_monstertail_07",
    [51] = "Ability_Hunter_Aspectofthemonkey",
    [52] = "inv_jewerly_necklace_22",
    [53] = "inv_scarab_silver",
    [55] = "Ability_Hunter_Pet_Spider",
    [56] = "achievement_boss_patchwerk",
    [57] = "inv_scarab_stone",
    [59] = "Ability_Hunter_Pet_Silithid",
    [66] = "Ability_Hunter_Pet_Wasp",
    [68] = "trade_archaeology_whitehydrafigurine"
}
...
local auralist = {
        ...
        [49039]     = IMMUNITY,     -- Lichborne
        [53534]     = ROOT,         -- Chains of Ice
        [108194]    = STUN,         -- Asphyxiate
        [118009]    = IMMUNITY,     -- Desecrated Ground
        [115001]    = STUN,          -- Remorseless Winter
        
        -- Druid
        [45334]     = ROOT,         -- Feral Charge
        [102359]    = ROOT,         -- Mass Entanglement
        [99]        = CROWDC,       -- Incapacitating Roar,
        ...
        -- Monk
        [137461]    = SILENCE,      -- Disarm from Ring of Peace
        [137460]    = SILENCE,      -- Silence from Ring of Peace
        [119392]    = STUN,         -- Charging Ox Wave
        [119381]    = STUN,         -- Leg Sweep
        [116706]    = ROOT,         -- Disable
        [117368]    = SILENCE,      -- Grapple Weapon
        [115078]    = CROWDC,       -- Paralysis
        [116709]    = SILENCE,      -- Spear Hand Strike
        [123393]    = CROWDC,       -- Breath of Fire
        [122242]    = STUN,         -- Clash
...
local function IsArenaPlayerByGUID(guid)
    for _, p in pairs(ATPlayers) do
        if p.guid == guid then
            return true
        end
...
local function CreateFrameForPlayer(p)
    ...
    if not IsArenaPlayerByGUID(p.guid)  then
        return
    end
    ...
    local scastt = scast:CreateTexture("CastBarTexture" .. globalCastbar, "ARTWORK", nil, 7)
    ...
    local tcastt = tcast:CreateTexture("CastBarTexture" .. globalCastbar, "ARTWORK", nil, 7)
...
local function UpdateValue(target, field, value)
    ...
    if (field == "health") then
        ...
        if (ATPlayers[target].status == 0) then
            ...
            SendDebugMessage("health" .. ATPlayers[target].health .. " maxhealth " .. ATPlayers[target]["maxhealth"])
...
local function LeaveArenaUI()
    --LeaveBattlefield()
    SendDebugMessage("leave")
    SendChatMessage(".spectate leave");
end
...
function ArenaSpectator.Load()
    ...
    leavearena:SetPoint("TOPLEFT", 130, 0)
    ...
    resetview:SetPoint("TOPLEFT", 260, 0)
    ...
    fixinterface:SetPoint("TOPLEFT", 390, 0)
